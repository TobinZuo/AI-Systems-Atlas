...
describe("Atlas hash router", () => {
  ...
  it("routes all available distributed playgrounds as topics", () => {
    ...
    assert.deepEqual(parseHash("#/distributed/collective"), { kind: "topic", topicId: "collective" });
    assert.deepEqual(parseHash("#/distributed/ring-allreduce"), { kind: "topic", topicId: "ring-allreduce" });
  ...
  it("routes the Autograd, Gradient, and AdamW playgrounds", () => {
    ...
    assert.deepEqual(parseHash("#/training/gradient"), { kind: "topic", topicId: "gradient" });
